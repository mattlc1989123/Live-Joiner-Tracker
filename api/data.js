 ...
 * Runs server-side on Vercel. Queries Tableau every time the dashboard loads.
 *
  ...
  const res = await fetch(`${HOST}/api/v1/vizql-data-service/query-datasource`, {
    ...
    body: JSON.stringify({
      ...
      query: {
        ...
        filters,
        paginate: { pageNumber: 1, pageSize: 100000 }
      },
      options: { returnFormat: 'OBJECTS', disaggregate: true }
    })
  });
  if (!res.ok) throw new Error(`VizQL query failed: ${res.status} ${(await res.text()).slice(0,200)}`);
...
export default async function handler(req, res) {
  ...
  if (!PAT || !SECRET) {
    return res.status(503).json({ error: 'Tableau credentials not configured. Add TABLEAU_PAT_NAME and TABLEAU_PAT_SECRET in Vercel Environment Variables.' });
  }
  ...
  try {
    ...
    const [clubRows, dayRows, heatRows] = await Promise.all([
      ...
      query(token, [
        ...
        MONTH_FILTER,
        { field: { fieldCaption: 'TM Joiners-' }, filterType: 'QUANTITATIVE_NUMERICAL', quantitativeFilterType: 'MIN', min: 1 }
      ])
    ]);
    ...
    const days = dayRows
      .map(d => ({ day: d.day, joiners: d.joiners ?? 0, target: Math.round(d.target ?? 0), ly: Math.round(d.ly ?? 0) }))
      .sort((a, b) => a.day - b.day);

    const heatMap = {};
    heatRows.forEach(h => {
      if (!heatMap[h.club]) heatMap[h.club] = { name: h.club, mgr: h.manager?.includes('Kris') ? 'kris' : 'lois', d: {} };
      heatMap[h.club].d[String(h.day)] = h.joiners ?? 0;
    });

    return res.status(200).json({
      today, monthName, monthNum, year, daysInMonth,
      pace: Math.round(pace * 10000) / 10000,
      refreshedAt,
      clubs,
      days,
